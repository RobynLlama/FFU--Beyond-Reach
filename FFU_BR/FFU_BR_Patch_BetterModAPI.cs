...
public static partial class patch_DataHandler {
    ...
    private static void SyncLoadMods(List<string[]> refQueuedPaths, string[] aIgnorePatterns) {
        ...
        foreach (string dataPath in validDataPaths) SyncLoadJSONs(dataPath, "verbs/", DataHandler.dictJsonVerbs, aIgnorePatterns);
        foreach (string dataPath in validDataPaths) SyncLoadJSONs(dataPath, "tokens/", DataHandler.dictJsonTokens, aIgnorePatterns);
        foreach (string dataPath in validDataPaths) SyncLoadJSONs(dataPath, "info/", DataHandler.dictInfoNodes, aIgnorePatterns);
        foreach (string dataPath in validDataPaths) SyncLoadJSONs(dataPath, "installables/", DataHandler.dictInstallables, aIgnorePatterns);
        foreach (string dataPath in validDataPaths) SyncLoadJSONs(dataPath, "interaction_overrides/", DataHandler.dictIAOverrides, aIgnorePatterns);
        foreach (string dataPath in validDataPaths) SyncLoadJSONs(dataPath, "plot_beat_overrides/", DataHandler.dictPlotBeatOverrides, aIgnorePatterns);
