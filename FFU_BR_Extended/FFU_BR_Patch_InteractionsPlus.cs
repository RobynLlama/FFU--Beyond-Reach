...
public partial class patch_Interaction : Interaction {
    ...
    [MonoModReplace] public void ApplyLogging(string strOwner, bool bTraitSuffix) {
        string strMsg = null;
        if (bWriteToLog) {
            strMsg = GrammarUtils.GenerateDescription(this, true);
            UnityEngine.Debug.Log($"#Interaction# {strName} (US: {objUs?.strName ?? "N/A"}, " +
                $"THEM: {objThem?.strName ?? "N/A"}, 3RD: {obj3rd?.strName ?? "N/A"}) => {strMsg}");
        }
        if (nLogging == Logging.NONE || bLogged) return;
        strMsg = strMsg ?? GrammarUtils.GenerateDescription(this, true);
