...
public partial class patch_ConsoleResolver : ConsoleResolver {
    [MonoModReplace] private static bool KeywordGetCond(ref string strInput, string[] strings) {
        ...
        if (cOwner != null && currData != string.Empty) {
            ...
            if (cOwner.IsThreshold(currData)) {
                string strCondName = currData.Substring(6);
                CondRule refCond = cOwner.GetCondRule(strCondName);
                if (refCond != null) {
                    strInput += $"\n{cOwner.strNameFriendly} ({cOwner.strName}) {currData} = x{refCond.Modifier}";
                    return true;
                }
            }
            bool isFirst = true;
...
private static bool KeywordGetCond(ref string strInput, string[] strings)
{
	...
	if (value != null && text != string.Empty)
	{
		...
		if (value.IsThreshold(text))
		{
			...
			CondRule condRule = value.GetCondRule(strCond);
			if (condRule != null)
			{
				flag = true;
				string text3 = strInput;
				strInput = text3 + "\n" + value.strNameFriendly + "." + text + " = " + condRule.Modifier;
			}
		}
		else
		{
