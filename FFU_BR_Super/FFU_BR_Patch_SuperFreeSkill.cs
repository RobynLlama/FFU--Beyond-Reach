...
public partial class patch_GUIChargenCareer : GUIChargenCareer {
    ...
    private void RebuildMultiSelectSidebar() {
        ...
        string yearsStr = DataHandler.GetString("GUI_CAREER_SIDEBAR_COST_2");
        int yearsTotal = 0;
        SpawnSideBarHeader();
        foreach (SkillSelectionDTO selectedSkill in _selectedSkills) {
            ...
            stringBuilder.AppendLine(yearsStr);
        ...
        textCompRefB.text = "<b>Total Cost: </b>" + yearsTotal + yearsStr;
...
private void RebuildMultiSelectSidebar()
{
	...
	LayoutRebuilder.ForceRebuildLayoutImmediate(tfSidebar.GetComponent<RectTransform>());
	string text = DataHandler.GetString("GUI_CAREER_SIDEBAR_COST_2");
	int num = 0;
	SpawnSideBarHeader();
	foreach (SkillSelectionDTO selectedSkill in _selectedSkills)
	{
	...
	component3.text = "<b>Total Cost: </b>" + num + text;
